...
{C2}            JSON + HTML Support • Telegram Report • Cyber UI{RS}
"""
    print(logo)
    print(f"{C2}{'═' * 74}{RS}")
...
def extract_usernames_from_instagram_html(path: str) -> Set[str]:
    ...
    # Ambil dari href dan text anchor
    ...
    # Fallback regex untuk pola instagram.com/username
    ...
    # Fallback regex untuk @username
    at_hits = re.findall(r"@([A-Za-z0-9._]{1,30})", html)
    for hit in at_hits:
        username = normalize_username(hit)
        if username:
            usernames.add(username)
...
def resolve_input_files() -> Tuple[str, str, str, str]:
    """
    Return:
    followers_path, followers_type, following_path, following_type
    type = 'json' or 'html'
    """
    followers_path = None
    followers_type = None
    ...
    if not followers_path:
        raise FileNotFoundError(
            f"Tidak menemukan {FOLLOWERS_JSON} atau {FOLLOWERS_HTML}"
        )
    if not following_path:
        raise FileNotFoundError(
            f"Tidak menemukan {FOLLOWING_JSON} atau {FOLLOWING_HTML}"
        )
...
def main() -> None:
    ...
    while True:
        ...
        print(f"{C3}│{RS} {C4}[5]{RS} Bantuan                                                          {C3}│{RS}")
        print(f"{C3}│{RS} {C4}[6]{RS} Keluar                                                           {C3}│{RS}")
        print(f"{C3}└──────────────────────────────────────────────────────────────────────┘{RS}")
