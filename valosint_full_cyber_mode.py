...
def checker_line(label: str, value: str, color: str = G):
...
def cyber_header(title: str):
    print()
    print(f"{M}{'┏' + '━' * 78 + '┓'}{RS}")
    print(f"{M}┃{RS} {BR}{W}{title.center(76)}{RS} {M}┃{RS}")
    print(f"{M}{'┗' + '━' * 78 + '┛'}{RS}")
...
╚██╗ ██╔╝██╔══██║██║     ██║   ██║╚════██║██║██║╚██╗██║   ██║
 ╚████╔╝ ██║  ██║███████╗╚██████╔╝███████║██║██║ ╚████║   ██║
  ╚═══╝  ╚═╝  ╚═╝╚══════╝ ╚═════╝ ╚══════╝╚═╝╚═╝  ╚═══╝   ╚═╝
{RS}{M}{BR}                  LIVE CYBER MODE • JSON + HTML + TELEGRAM{RS}
{B}             LOST / NEW / NO-BACK / FOLLOW • AUTO BASELINE SYSTEM{RS}
""")
    print(f"{CY}{'═' * 82}{RS}")
...
def auto_promote_baseline():
    if os.path.exists(CURRENT_SNAPSHOT):
        shutil.copyfile(CURRENT_SNAPSHOT, PREVIOUS_SNAPSHOT)
        status(f"Auto baseline updated -> {PREVIOUS_SNAPSHOT}", "ok")
...
def process_live_section(title: str, usernames: Set[str], cache: Dict[str, dict], label: str, color: str):
    ...
    print(f"{B}[*]{RS} Scanning {len(data)} accounts...\n")
    for i, username in enumerate(data, start=1):
        profile = get_profile_info(username, cache)
...
def compare_old_and_new(send_to_telegram: bool = False):
    ...
    if not os.path.exists(PREVIOUS_SNAPSHOT):
        status("Baseline lama belum ada, sistem akan membuat baseline otomatis dari snapshot terbaru.", "warn")
        auto_promote_baseline()
        status("Sekarang jalankan menu 1 lagi saat data berubah, lalu menu 2/3 untuk compare.", "ok")
        return
    ...
    process_live_section("NEW FOLLOWING", result["new_following_added"], cache, "FOLLOW", CY)
...
def show_help():
    ...
    print(f"{B}- Baseline lama sekarang AUTO dibuat / diupdate{RS}")
